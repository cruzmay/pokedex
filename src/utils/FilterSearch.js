...
export const FilterSearch = () => {
    ...
    // console.log(orderedPokemons.filter( data => data.gender.some( c => c.includes('male'))))
    ...
    const handleFilters = () => {
        if(filteredType.length > 0 && filteredCol.length === 0 && filteredGender.length === 0 ) {
        ...
        } else if ( filteredType.length === 0 && filteredCol.length === 0 && filteredGender.length > 0 && filteredGender !== 'all'  ) {
            //filtered by gender
            ...
            return orderedPokemons.filter(data => data.name.includes(searchPokemon) 
                || data.color.includes(searchPokemon) 
                || data.type.includes(searchPokemon) 
                || data.description.includes(searchPokemon) 
                || data.id === parseInt(searchPokemon) )
        } else {
            // all pokemons 
